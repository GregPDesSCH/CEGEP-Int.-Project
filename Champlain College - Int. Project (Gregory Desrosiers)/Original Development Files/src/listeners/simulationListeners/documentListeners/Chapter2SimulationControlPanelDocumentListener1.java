...
public class Chapter2SimulationControlPanelDocumentListener1 extends
		PlainDocument 
{
			  ...
			  throws BadLocationException 
	{
		...
		int numberOfDecimalPoints = 0, numberOfMinusSigns = 0;
		
		for (int b = 0; b < getLength(); b++)
			if (getText(0, getLength()).charAt(b) == '.')
				numberOfDecimalPoints++;
			else if (getText(0, getLength()).charAt(b) == '-')
				numberOfMinusSigns++;
		...
		{	
			...
			if ((str.charAt(x) < 48 || str.charAt(x) > 57) && str.charAt(x) != '.' 
					&& str.charAt(x) != '-')
				return;
		}
		
		if ((getLength() + str.length()) <= (STRING_MAX_SIZE + 1) && numberOfMinusSigns <= 1 
				&& str.equals("-"))
			super.insertString(0, str, attr);
		else if (((getLength() - str.length()) <= STRING_MAX_SIZE && numberOfMinusSigns > 1 
				&& str.equals("-")))
			super.remove(0, 1);
		else if ((getLength() + str.length()) <= STRING_MAX_SIZE && numberOfDecimalPoints <= 1 
				&& numberOfMinusSigns <= 1) 
